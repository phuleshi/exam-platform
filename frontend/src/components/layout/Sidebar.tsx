...
import { LayoutDashboard, FileSpreadsheet, Award, PlusCircle, HelpCircle, Users } from 'lucide-react';
...
export const Sidebar: React.FC = () => {
  ...
  const teacherLinks = [
    ...
    { to: '/teacher/classes', label: 'Quản Lý Lớp Học', icon: Users },
  ...
  return (
    <aside className="w-64 border-r border-slate-200 bg-white p-4 flex flex-col justify-between shrink-0 min-h-[calc(100vh-4rem)]">
      <div className="flex flex-col gap-1">
        <span className="px-3 text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">
          {isTeacher ? 'Danh Mục Giáo Viên' : 'Danh Mục Học Sinh'}
        </span>
        {links.map((link) => {
